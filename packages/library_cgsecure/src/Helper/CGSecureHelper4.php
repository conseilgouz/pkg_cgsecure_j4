...
class CGSecureHelper4
{
    ...
    public static function forceHTAccess($json = false)
    {
        ...
        if (self::merge_file(self::getServerConfigFilePath(self::SERVER_CONFIG_FILE_HTACCESS), $current, $cgFile, $rejips, $specific, $cgAI, $hotlink)) {
            if ($json) {
    ...
    public static function protectotherdirs($json = false)
    {
        ...
        if (!self::merge_file($dest, $current, $cgFile, '')) {
            if ($json) {
